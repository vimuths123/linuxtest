        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    ...
                    <form method="post" action="{{ route('issues.store') }}" class="mt-6 space-y-6" x-data="{ purchaseProductId: '{{ old('purchase_product_id') }}', freeProductId: '{{ old('free_product_id') }}' }">
                        @csrf
                        <div>
                            <x-input-label for="label" :value="__('Free issue Label')" />
                            <x-text-input id="label" name="label" type="text" class="mt-1 block w-full" :value="old('label')" autofocus autocomplete="label" />
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="p-2 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                                ...
                                <option value="flat" x-bind:selected="type === 'flat' || '{{ old('type') }}' === 'flat'">Flat</option>
                                <option value="multiple" x-bind:selected="type === 'multiple' || '{{ old('type') }}' === 'multiple'">Multiple</option>
                            </select>
                            <x-input-error :messages="$errors->get('type')" />
                        </div>

                        <div til="{{ old('purchase_product_id') }}">
                        ...
                        <div>
                            ...
                            <x-text-input id="purchase_quantity" name="purchase_quantity" type="text" class="mt-1 block w-full" :value="old('purchase_quantity')" autofocus autocomplete="purchase_quantity" />
                            <x-input-error class="mt-2" :messages="$errors->get('purchase_quantity')" />
                        </div>

                        <div>
                            <x-input-label for="free_quantity" :value="__('Free Quantity')" />
                            <x-text-input id="free_quantity" name="free_quantity" type="text" class="mt-1 block w-full" :value="old('free_quantity')" autofocus autocomplete="free_quantity" />
                            <x-input-error class="mt-2" :messages="$errors->get('free_quantity')" />
                        </div>

                        <div>
                            <x-input-label for="lower_limit" :value="__('Lower Limit')" />
                            <x-text-input id="lower_limit" name="lower_limit" type="text" class="mt-1 block w-full" :value="old('lower_limit')" autofocus autocomplete="lower_limit" />
                            <x-input-error class="mt-2" :messages="$errors->get('lower_limit')" />
                        </div>

                        <div>
                            <x-input-label for="upper_limit" :value="__('Upper Limit')" />
                            <x-text-input id="upper_limit" name="upper_limit" type="text" class="mt-1 block w-full" :value="old('upper_limit')" autofocus autocomplete="upper_limit" />
